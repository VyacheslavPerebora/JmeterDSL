...
public class LogoutTest {
    ...
    static final Logger logger = Logger.getLogger(LogoutTest.class);
    ...
    private void test() throws IOException, InterruptedException, TimeoutException {
        ...
        TestPlanStats run = testPlan(
                ...
                getHeaders(),
                getHttpDefaults(),
                TicketCreationThreadGroup.getThreadGroup("TG_LOGOUT", debugEnable)
                        .children(
                                ifController(s -> !debugEnable,
                                        testAction(throughputTimer(throughputPerMinute).perThread())
                                ),
                                transaction("UC_LOGOUT",
                                        logoutFragment.get()
                                )
                        ),
